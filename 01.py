...
def grayscale(img, r=0.299, g=0.587, b=0.114):
    ...
    def gray_pix_fun(pix):
        return (floor(get_color_mean(pix, r, g, b)),) * 3

    return Image.image_map(gray_pix_fun, img)
...
def desaturate(img, s=0.5, r=0.299, g=0.587, b=0.114):
    ...
    def des_pix_fun(gs, origin):
        y = gs[0]
        return tuple([floor(y + (s*(color - y))) for color in origin])

    with grayscale(img, r, g, b) as gs:
        new = Image.image_map(des_pix_fun, gs, img)
